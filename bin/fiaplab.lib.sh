...
show_vm_status() {
    ...
    if [ "$FIAP" = "0" ]; then
        ...
        echo "Code-server : indisponível (use a opção 5 → Refazer, ou init.sh)"
    fi
...
# tokens temporarios que expiram, entao o terraform/aws na VM comeca
# a dar "ExpiredToken". Quem usa o code-server pela web nao passa
